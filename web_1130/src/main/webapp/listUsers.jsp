...
<%@ page import="org.yty.UserInfo" %>
<%@ page import="org.yty.Biz.UserInfoBiz" %>
...
<%
    //获得数据
    List<UserInfo>  lists=(List<UserInfo>) request.getAttribute("lists");
    //循环
    for(UserInfo user:lists){
%>
                <tr align="center">
                <td width="25%"><a href="userInfo.jsp?id=<%=user.getId()%>&uname=<%=user.getUsername()%>&upwd=<%=user.getPassword()%>"><%=user.getId()%></a></td>
    ...
    <tr>
        <td align="right"><a href="listUsers.html">首页</a>||<a href="listUsers.html">上页</a>||<a href="listUsers.html">下页</a>||<a href="listUsers.html">末页</a></td>
    </tr>
    <tr>
